...
//            EventBus.getDefault().post(mLocationEvent);
    ...
    public void setLocationAddress(String current_location_txt  ) {
        if(current_location_txt == null || current_location_txt.equals("null")){
            mLocationEvent.setLocationWithAddress(getLocationDetails().get(KEY_CURRENT_LAT), getLocationDetails().get(KEY_CURRENT_LONG) , ""+_context.getResources().getString(com.taas.driver.R.string.no_internet_connection), Float.parseFloat(getLocationDetails().get(LocationSession.KEY_BEARING_FACTOR)));
            EventBus.getDefault().post(mLocationEvent);
           }
        else{
