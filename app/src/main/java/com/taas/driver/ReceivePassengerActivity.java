...
public class ReceivePassengerActivity extends Activity implements ApiManager.APIFETCHER {
   ...
   // FirebaseUtils firebaseUtils;
    ...
    TextView timeTxt;
    @Bind(com.taas.driver.R.id.cancel_btn)
    LinearLayout cancelBtn;
    @Bind(com.taas.driver.R.id.cancel_layout)
    LinearLayout cancelLayout;
    @Bind(com.taas.driver.R.id.main_layout_payment_mode)
    TextView mainLayoutPaymentMode;
    @Bind(com.taas.driver.R.id.main_layout_payment)
    TextView mainLayoutPayment;
    @Bind(com.taas.driver.R.id.map_image)
    CircleImageView mapImage;
    @Bind(com.taas.driver.R.id.main_layout_pickup_txt)
    TextView mainLayoutPickupTxt;
    @Bind(com.taas.driver.R.id.main_layout)
    LinearLayout mainLayout;
    @Bind(com.taas.driver.R.id.ride_expire_pick_address_txt)
    TextView rideExpirePickAddressTxt;
    @Bind(com.taas.driver.R.id.ride_expire_drop_address_txt)
    TextView rideExpireDropAddressTxt;
    @Bind(com.taas.driver.R.id.expire_msg)
    TextView expireMsg;
    @Bind(com.taas.driver.R.id.ride_expire_ok_btn)
    LinearLayout rideExpireOkBtn;
    @Bind(com.taas.driver.R.id.ride_expire_layout)
    LinearLayout rideExpireLayout;
    @Bind(com.taas.driver.R.id.accept_ride_btn)
    LinearLayout acceptRideBtn;
    @Bind(com.taas.driver.R.id.cash_layout)
    LinearLayout cash_Layout;

    @Bind(com.taas.driver.R.id.pulsator)
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(com.taas.driver.R.layout.activity_receive_passenger);
    ...
    private void setMediaSound() throws Exception {
        ...
        mediaPlayer.setDataSource(this, Uri.parse("android.resource://" + this.getPackageName() + "/" + com.taas.driver.R.raw.message_pops));
    ...
    public void onFetchComplete(Object script, String APINAME) {
        try {
            if (APINAME.equals("" + Config.ApiKeys.KEY_VIEW_RIDE_INFO_DRIVER)) {
                ...
                Glide.with(this).load("" + Apis.googleImage + "" + viewRideInfoDriver.getDetails().getPickup_lat() + "," + viewRideInfoDriver.getDetails().getPickup_long() + "&zoom=15&size=400x400&key=" + ReceivePassengerActivity.this.getResources().getString(com.taas.driver.R.string.google_map_key)).into(mapImage);
                ...
                if (viewRideInfoDriver.getDetails().getPayment_option_id().toString().equals("1")) {
                    mainLayoutPayment.setText(getResources().getString(com.taas.driver.R.string.WEEKLY_STATEMENT_ACTIVITY__cash));
                } else if (viewRideInfoDriver.getDetails().getPayment_option_id().toString().equals("2")) {
                    mainLayoutPayment.setText(getResources().getString(com.taas.driver.R.string.WEEKLY_STATEMENT_ACTIVITY__paypal));

                } else if (viewRideInfoDriver.getDetails().getPayment_option_id().toString().equals("3")) {
                    mainLayoutPayment.setText(getResources().getString(com.taas.driver.R.string.WEEKLY_STATEMENT_ACTIVITY__card));

                } else if (viewRideInfoDriver.getDetails().getPayment_option_id().toString().equals("4")) {
                    mainLayoutPayment.setText(getResources().getString(com.taas.driver.R.string.WEEKLY_STATEMENT_ACTIVITY__wallet));
    ...
    public void onMessageEvent(RideSessionActiveRideEvent event) {
        if (event.getRide_status().equals("104")){
            if (!RIDE_BTN_CLICKED == true) {
                ...
                setViewAccordingToRideStatus(event.getRide_status(), getResources().getString(com.taas.driver.R.string.ride_cancel));
                mediaPlayer.stop();
                finish();
