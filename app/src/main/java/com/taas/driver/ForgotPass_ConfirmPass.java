...
public class ForgotPass_ConfirmPass extends AppCompatActivity implements ApiManager.APIFETCHER {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(com.taas.driver.R.layout.activity_forgotpass_changepass);
        //progress bar
        pd = new ProgressDialog(this);
        pd.setMessage(ForgotPass_ConfirmPass.this.getResources().getString(com.taas.driver.R.string.loading));
        pd.setCancelable(false);
        pd.setCanceledOnTouchOutside(false);

        new_pass = (EditText) findViewById(com.taas.driver.R.id.edit_newPass);
        confirm_pass = (EditText) findViewById(com.taas.driver.R.id.edit_confrmPass);
        register_password = (LinearLayout) findViewById(com.taas.driver.R.id.register_password1);
        ...
        if (APINAME.equals("" + Config.ApiKeys.KEY_FORGOTPASS_CONFIRMPASS)) {
            try {
                ...
                if (rs.result.equals("1")) {
                    Toast.makeText(this, com.taas.driver.R.string.changed_password, Toast.LENGTH_SHORT).show();
